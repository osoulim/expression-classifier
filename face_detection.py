import cv2
import glob
face_detector1 = cv2.CascadeClassifier(
    "certificates/haarcascade_frontalface_default.xml")
face_detector2 = cv2.CascadeClassifier(
    "certificates/haarcascade_frontalface_alt2.xml")
face_detector3 = cv2.CascadeClassifier(
    "certificates/haarcascade_frontalface_alt.xml")
face_detector4 = cv2.CascadeClassifier(
    "certificates/haarcascade_frontalface_alt_tree.xml")

emotion_list = ["neutral", "anger", "contempt",
                "disgust", "fear", "happy", "sadness", "surprise"]


def faceDetection(emotion):
    # Get list of all images with emotion
    files = glob.glob("selected_set\\%s\\*" % emotion)
    filenumber = 0
    for f in files:
        ...
        for (x, y, w, h) in facefeatures:  # get coordinates and size of rectangle containing face
            ...
            try:
                ...
                cv2.imwrite("final_dataset\\%s\\%s.jpg" %
